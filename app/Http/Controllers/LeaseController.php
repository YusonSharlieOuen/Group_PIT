...
class LeaseController extends Controller
{
    ...
    {
        // Safe string variable resolution to bypass custom text primary key (e.g., 'L1') binding bugs
        $leaseId = $id instanceof Lease ? $id->lease_id : $id;
        $lease = Lease::where('lease_id', $leaseId)->firstOrFail();

        $formData = $this->getFormData();
        
        // Match the directory name casing strictly to prevent 500 Finder errors
        return view('Lease.edit_lease', array_merge(compact('lease'), $formData));
    }
